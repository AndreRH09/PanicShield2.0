    ...
    class HomeViewModel @Inject constructor(
        ...
        private fun initializeSms() {
            viewModelScope.launch {
                try {
                    ...
                    // ✅ USAR AMBAS VERIFICACIONES DE PERMISOS
                    val hasSmsPermission = smsHelper.hasSmsPermission()
                    val hasAllPermissions = _permissionsGranted.value
    
                    Log.d("HomeViewModel", "📱 Permisos SMS (SmsHelper): $hasSmsPermission")
                    Log.d("HomeViewModel", "📱 Permisos All (PermissionManager): $hasAllPermissions")
    
                    // ✅ ACTUALIZAR AMBOS ESTADOS
                    _uiState.value = _uiState.value.copy(hasSmsPermission = hasSmsPermission)
                    _permissionsGranted.value = hasSmsPermission && hasAllPermissions
    
                    Log.d("HomeViewModel", "📱 Estado final permisos: ${_permissionsGranted.value}")
        ...
        fun updatePermissionStatus(context: Context) {
            ...
            Log.d("HomeViewModel", "🔄 Actualizando permisos:")
            Log.d("HomeViewModel", "   📱 SMS Permission: $hasSmsPermission")
            Log.d("HomeViewModel", "   🔐 All Permissions: $hasAllPermissions")
    
            // ✅ COMBINAR AMBAS VERIFICACIONES
            val finalPermissionStatus = hasSmsPermission && hasAllPermissions
            _permissionsGranted.value = finalPermissionStatus
    
            _uiState.value = _uiState.value.copy(hasSmsPermission = hasSmsPermission)
    
            Log.d("HomeViewModel", "✅ Estado final permisos: $finalPermissionStatus")
        }
        fun syncPermissionStatus(context: Context) {
            viewModelScope.launch {
                try {
                    Log.d("HomeViewModel", "🔄 Sincronizando estado de permisos...")
                    ...
                    // ✅ LOGS DETALLADOS
                    Log.d("HomeViewModel", "📊 Estado actual de permisos:")
                    Log.d("HomeViewModel", "   📱 SMS (SmsHelper): $hasSmsPermission")
                    Log.d("HomeViewModel", "   🔐 All (PermissionManager): $hasAllPermissions")
                    Log.d("HomeViewModel", "   📊 Estado anterior: ${_permissionsGranted.value}")
    
                    // ✅ ACTUALIZAR ESTADOS
                    val newPermissionStatus = hasSmsPermission && hasAllPermissions
                    _permissionsGranted.value = newPermissionStatus
    
                    _uiState.value = _uiState.value.copy(
                        hasSmsPermission = hasSmsPermission,
                        isLocationPermissionGranted = hasAllPermissions
                    )
    
                    Log.d("HomeViewModel", "✅ Permisos sincronizados. Nuevo estado: $newPermissionStatus")
    
                } catch (e: Exception) {
                    Log.e("HomeViewModel", "❌ Error sincronizando permisos: ${e.message}", e)
                }
            }
        }
        ...
        private fun activateEmergencyWithPriority(priority: String) {
            viewModelScope.launch {
                ...
                // ✅ VERIFICACIÓN DOBLE DE PERMISOS SMS
                val hasSmsFromHelper = smsHelper.hasSmsPermission()
                val finalSmsPermission = hasSmsFromHelper
    
                Log.d("HomeViewModel", "📱 Verificación SMS:")
                Log.d("HomeViewModel", "   SmsHelper: $hasSmsFromHelper")

                Log.d("HomeViewModel", "   Final: $finalSmsPermission")
                ...
                try {
                    ...
                    when (result) {
                        is EmergencyResult.Success -> {
                            ...
                            // ✅ USAR VERIFICACIÓN MEJORADA DE PERMISOS SMS
                            if (finalSmsPermission) {
                                Log.d("HomeViewModel", "📱 Enviando SMS con permisos verificados")
                                sendSmsAlertsWithPriority(currentLocation, priority)
                            } else {
                                Log.w("HomeViewModel", "⚠️ Sin permisos SMS completos")
                                _uiState.value = _uiState.value.copy(
                                    errorMessage = "Emergencia activada. SMS no enviado: permisos SMS requeridos",
                                    smsStatus = "❌ SMS no enviado: faltan permisos"
                                )
                            }
        ...
        private suspend fun sendSmsAlertsWithPriority(currentLocation: LocationInfo, priority: String) {
            try {
                ...
                // ✅ CAMBIO: Usar la misma verificación que en activateEmergencyWithPriority
                val hasSmsPermission = smsHelper.hasSmsPermission()
                ...
                if (!smsHelper.isConnected()) {
                    ...
                    if (!connected) {
                        ...
                        _uiState.value = _uiState.value.copy(
                            smsStatus = "❌ SMS no enviado: error de conexión"
                        )
                        return
                    }
                }
                ...
                if (contactsResult.isSuccess) {
                    ...
                    if (contacts.isNullOrEmpty()) {
                        ...
                        _uiState.value = _uiState.value.copy(
                            smsStatus = "⚠️ No hay contactos para notificar"
                        )
                        return
                    }
                    ...
                    _uiState.value = _uiState.value.copy(
                        smsStatus = "❌ Error obteniendo contactos"
                    )
                }
            } catch (e: Exception) {
                Log.e("HomeViewModel", "❌ Error general enviando SMS: ${e.message}", e)
                _uiState.value = _uiState.value.copy(
                    smsStatus = "❌ Error enviando SMS: ${e.message}"
                )
            }
        }
        ...
        private suspend fun sendSmsAlerts(currentLocation: LocationInfo) {
            try {
                ...
                // ✅ CAMBIO: Usar smsHelper.hasSmsPermission() directamente
